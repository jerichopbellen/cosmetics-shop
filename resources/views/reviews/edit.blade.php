        ...
        <div class="col-lg-4 mb-4">
            <div class="card border-0 shadow-sm text-center p-4 h-100">
                <div class="mb-3 mx-auto">
                    {{-- Product Image from the product relationship --}}
                    <img src="{{ asset('storage/' . ($review->product->images->first()->image_path ?? 'placeholders/product.png')) }}" 
                         class="rounded border border-pink p-1 shadow-sm" 
                         style="width: 100%; max-width: 200px; aspect-ratio: 1/1; object-fit: cover;">
                </div>
                
                <h4 class="fw-bold mb-1 text-dark">{{ $review->product->name }}</h4>
                <p class="text-muted small text-uppercase fw-bold mb-2" style="letter-spacing: 1px;">
                    {{ $review->product->category->name ?? 'Cosmetics' }}
                </p>

                {{-- Displaying the shade name from your column --}}
                <div class="mb-4">
                    <span class="badge bg-light text-pink border border-pink-subtle px-3 py-2 rounded-pill shadow-sm">
                        <i class="fa-solid fa-palette me-1"></i> Shade: {{ $review->shade->shade_name ?? 'Default' }}
                    </span>
                </div>
                ...
                <div class="text-start mt-3">
                    ...
                    <ul class="text-muted ps-3 mb-0" style="font-size: 0.85rem;">
                        ...
                        <li>Updating your photo helps others see the results!</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm overflow-hidden">
                ...
                <div class="card-body p-4 p-md-5">
                    <h3 class="fw-bold mb-4 text-dark">Edit Your Review</h3>
                    ...
                    <form action="{{ route('reviews.update', $review->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="comment" id="comment" rows="5" 
                                ...
                                placeholder="What did you love about this product?" required>{{ old('comment', $review->comment) }}</textarea>
                            @error('comment') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Image Upload Zone --}}
                        <div class="mb-4">
                            <label class="form-label small fw-bold text-muted text-uppercase">Review Photo</label>
                            
                            {{-- Input name is usually 'photo' or 'image' to match your Request validation --}}
                            ...
                            <div class="upload-zone rounded text-center cursor-pointer border-pink-subtle" id="upload-trigger" onclick="document.getElementById('photo-input').click()">
                                ...
                                {{-- Prompt: Hidden if photo_path exists --}}
                                <div id="upload-prompt" class="p-4 {{ $review->photo_path ? 'd-none' : '' }}">
                                ...
                                {{-- Preview: Shown if photo_path exists --}}
                                <div id="preview-wrapper" class="p-2 {{ $review->photo_path ? '' : 'd-none' }}">
                                    <div class="position-relative d-inline-block">
                                        <img id="review-preview" 
                                             src="{{ $review->photo_path ? asset('storage/' . $review->photo_path) : '#' }}" 
                                             class="rounded border border-pink p-1 shadow-sm" 
                                    ...
                                    <div class="mt-2 text-pink small fw-bold">Click box to change or replace image</div>
                                </div>
                            </div>
                        ...
                        <div class="text-end mt-5">
                            ...
                            <input type="hidden" name="shade_id" value="{{ $review->shade_id }}">
...
<style>
    ...
    .border-pink-focus:focus { border-color: #ec4899; box-shadow: 0 0 0 0.25rem rgba(236, 72, 153, 0.1); }
    .border-pink-subtle { border-color: #fce7f3; }
    .upload-zone { background-color: #fffdfd; border: 2px dashed #fce7f3; transition: 0.2s; cursor: pointer; }
    .upload-zone:hover { background-color: #fff9fc; border-color: #ec4899; }
    .rating-stars input { display: none; }
    .star-label { transition: 0.2s; color: #dee2e6; cursor: pointer; }
    .star-label:hover, .star-label:hover ~ .star-label, .btn-check:checked ~ .star-label { color: #ffc107 !important; }
</style>
@endsection